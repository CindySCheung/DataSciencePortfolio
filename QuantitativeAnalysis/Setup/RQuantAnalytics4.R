...
####################################################################
# Fit White Noise Model on difference between today's price and
# yesterday's price
# If estimated mean = intercept = 0, then diff series is considered
# White Noise, which then signifies that the time series is likely
# a Random Walk.
####################################################################
...
diff_wn <- arima(x = close_diff, order = c(0, 0, 0))
...
####################################################################
# Random Walk Model
# Determine whether the adjusted closing price is a Random Walk.
# ARIMA results confirmed with slope of 0.9994 and ACF confirmed it
# with majority of lags at less than 5% level
####################################################################

arima(Adjusted, order = (c(1, 0, 0)))

acf(diff(Adjusted), na.action = na.omit)
acf(diff(Adjusted), na.action = na.omit, plot = FALSE)
...
acf(Adjusted_ts, plot = TRUE)
acf(Adjusted_ts, plot = FALSE)

acf(Adjusted_ts, lag.max = 500, plot = TRUE)
pacf(Adjusted_ts, plot = TRUE)
